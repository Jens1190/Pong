...
class TopBorder {
    ...
    init (scene:SKScene) {
        ...
        shapeNode = SKShapeNode(rect: CGRect(x: 0, y: scene.frame.height - 5, width: scene.frame.width, height: 5))
        
        shapeNode.fillColor = UIColor.redColor()
        shapeNode.lineWidth = 0
        
        physicsBody = SKPhysicsBody(edgeLoopFromRect: CGRect(x: 0, y: scene.frame.height - 5, width: shapeNode.frame.width, height: 5))
        ...
        physicsBody.categoryBitMask = BitMasks.TopBorder
